...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/verstka-agency/debug/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://raw.githubusercontent.com/verstka-agency/debug/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/verstka-agency/debug/master/install.sh)${plain}"
    echo ""
